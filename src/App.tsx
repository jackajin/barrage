...
const Home = () => {
  ...
  const initBarrage = (el: string) => {
    ...
    setInterval(() => {
      ref.current?.pushList(value.slice(index.current, index.current + 6))
      index.current += 6
      if (index.current > 50) {
        index.current = 0
      }
  ...
  return (
    <div>
      ...
      <div className='container' style={{ width: '100%', height: '150px', border: '1px solid black',background: 'red' }}></div>
